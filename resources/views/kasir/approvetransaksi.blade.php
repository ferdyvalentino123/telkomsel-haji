...
@push('scripts')
    ...
    <script>
        $(document).ready(function () {
            ...
            // Delegated: handle btn-bayar-checklist
            $(document).on('change', '.btn-bayar-checklist', function () {
                const checkbox = $(this);
                if (!checkbox.is(':checked')) return;

                const id = checkbox.data('id');
                const nama = checkbox.data('nama');

                Swal.fire({
                    title: 'Konfirmasi Pembayaran',
                    text: `Konfirmasi pembayaran untuk ${nama}?`,
                    icon: 'question',
                    ...
                    if (result.isConfirmed) {
                        window.location.href = `/programhaji/kasir/transaksi/${id}/bayar`;
                    } else {
                        checkbox.prop('checked', false); // Uncheck if cancelled
                    }
                });
            });
            ...
            $(document).on('click', '.btn-unlunas', function (e) {
                ...
                const actionUrl = `/programhaji/kasir/transaksi/kwitansi/unlunas/${transaksiId}`;
            ...
            $(document).on('click', '.btn-delete', function(e) {
                ...
                if (confirm(`Are you sure you want to permanently delete transaction for ${nama}?`)) {
                    $.ajax({
                        url: `/programhaji/kasir/transaksi/${id}/forcedelete`,
                        type: 'DELETE',
                        data: {
                        ...
                        success: function(response) {
                            ...
                            $.ajax({
                                url: '/programhaji/kasir/approvetransaksi/refresh',
                                type: 'GET',
                                dataType: 'json',
                                success: function(response) {
                                    table.clear();
                    ...
                    success: function (response) {
                        ...
                        response.transaksi.forEach(function (item) {
                            table.row.add([
                                ...
                                item.nama_sales ?? '-',
                                item.kasir?.name ?? '-',
                                item.sales?.tempat_tugas ?? item.sales?.bertugas ?? '-',
                                ...
                                `<div class="form-check d-flex justify-content-center">
                                    <input class="form-check-input btn-bayar-checklist" type="checkbox" 
                                ...
                                (function () {
                                    ...
                                    if (currentUser.is_superuser) {
                                        btns += `<a href="/programhaji/kasir/transaksi/${item.id}/edit" class="btn btn-warning btn-sm">Edit</a> `;
                                        btns += `<a href="#" class="btn btn-danger btn-sm btn-delete" data-id="${item.id}" data-nama="${item.nama_pelanggan}">Delete</a> `;
                                    }
                                    if (item.is_paid) {
                                        btns += `
                                            <a href="/programhaji/kasir/transaksi/kwitansi/print/${item.id}" class="btn btn-success btn-sm" target="_blank">Print</a>
                                            ${currentUser.is_superuser
                                                ? `<a href="#" class="btn btn-warning btn-sm btn-unlunas" data-id="${item.id}">Un-Lunas</a>`
                                                : ''}`;
                                    }
                                    return btns;
                                })()
