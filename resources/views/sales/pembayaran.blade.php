<x-Sales.SalesLayouts>
    ...
    <div class="payment-container">
        ...
        <div class="payment-header">
            ...
            <h1>Konfirmasi Pembayaran Sales</h1>
            <p>Selesaikan pembayaran Nontunai untuk pelanggan</p>
        </div>
        ...
        <button type="button" id="pay-button" class="btn-pay">
            <i class="fas fa-qrcode"></i> Tampilkan QRIS / Pembayaran
        </button>

        <div class="text-center mt-3">
            <form action="{{ route('sales.pembayaran.cancel', $transaksi->id) }}" method="POST">
                @csrf
                <button type="submit" class="btn btn-link text-muted text-decoration-none" onclick="return confirm('Anda yakin ingin membatalkan transaksi Nontunai ini? Stok akan dikembalikan dan Anda akan kembali ke form.')">
                    <i class="fas fa-arrow-left me-1"></i> Batalkan & Kembali ke Form
                </button>
            </form>
        </div>
    ...
    <script>
        ...
        payButton.addEventListener('click', function() {
            ...
            payButton.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Menunggu Pembayaran...';
            ...
            snap.pay(snapToken, {
                onSuccess: function(result) {
                    Swal.fire({
                        ...
                        text: 'Terima kasih atas pembayaran Nontunai.',
                        confirmButtonColor: '#21e23e'
                    }).then(() => {
                        window.location.href = '{{ route("sales.pembayaran.callback", $transaksi->id) }}';
                    });
                },
                onPending: function(result) {
                    Swal.fire({
                        ...
                        window.location.href = '{{ route("sales.transaksi") }}';
                    });
                },
                onError: function(result) {
                    ...
                    payButton.innerHTML = '<i class="fas fa-qrcode"></i> Tampilkan QRIS / Pembayaran';
                },
                onClose: function() {
                    payButton.disabled = false;
                    payButton.innerHTML = '<i class="fas fa-qrcode"></i> Tampilkan QRIS / Pembayaran';
                    Swal.fire({
                        icon: 'info',
                        title: 'Popup Ditutup',
                        text: 'Anda menutup layar pembayaran Midtrans.',
                        confirmButtonColor: '#bc0007'
                    });
                }
            });
        ...
        // Auto-trigger if we want it to show immediately (optional)
